// customized html-to-draft to allow align text
const CharacterMetadata = require("draft-js/lib/CharacterMetadata");
const ContentBlock = require("draft-js/lib/ContentBlock");
const ContentBlockNode = require("draft-js/lib/ContentBlockNode");
const DefaultDraftBlockRenderMap = require("draft-js/lib/DefaultDraftBlockRenderMap");
...
const { List, OrderedSet, Map } = Immutable;
  ...
  if (styleToCheck) {
    ...
    currentStyle = currentStyle
      .withMutations(style => {
        ...
        const textAlignment = htmlElement.style.textAlign;
        ...
        if (["left", "right", "center"].includes(textAlignment)) {
          style.add(`TEXTALIGN${textAlignment.toUpperCase()}`);
        }
      })
      .toOrderedSet();
  }
...
const processData = (tag, node, currentData) => {
